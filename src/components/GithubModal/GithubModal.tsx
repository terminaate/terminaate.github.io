...
import { AiOutlineLink, IoLocationSharp, FaUsers, AiOutlineClockCircle } from 'react-icons/all';
...
const GithubModal: FC<IGithubModal> = ({ modal, setModal }) => {
  ...
  const setAsyncState = (setState: React.Dispatch<React.SetStateAction<any>>, promise: Promise<AxiosResponse>) => {
    promise.then((r: AxiosResponse) => r.status === 200 ? setState(r.data) : '');
  };
        ...
        <MovableModal minWidth={'800px'} className={cl.githubModal} width={'50%'} title={'❤️My github️'}
          ...
          <div className={cl.userInfo}>
            ...
            <AnimatedSymbolsText onClick={() => open('//github.com/terminaate')} infinite={true}
                                 className={cl.userNickname} text={"T$rm1naate"}/>
